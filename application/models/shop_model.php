...
class Shop_model extends CI_Model
{
	...
	public function create($name,$alias,$status,$metatitle,$metadescription,$banner,$bannerdescription,$defaulttax,$user)
	{
		$data  = array(
			'name' => $name,
			'alias' => $alias,
...
//		}
	...
	function viewshop()
	{
		$query="SELECT `shop`.`id`, `shop`.`name`, `shop`.`user`, `shop`.`alias`, `shop`.`status`, `shop`.`metatitle`, `shop`.`metadescription`, `shop`.`bannername`, `shop`.`bannerdescription`, `shop`.`defaulttax`,`user`.`firstname`,`user`.`lastname` FROM `shop` LEFT OUTER JOIN `user` ON `shop`.`user`=`user`.`id`";
		$query=$this->db->query($query)->result();
		return $query;
	}
    
	function changeshopstatus($id)
	{
		$query=$this->db->query("SELECT `status` FROM `shop` WHERE `id`='$id'")->row();
		$status=$query->status;
		if($status==1)
		{
			$status=0;
	...
	{
		$this->db->where( 'id', $id );
		$query['shop']=$this->db->get( 'shop' )->row();
		return $query;
	}

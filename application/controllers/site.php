...
//            print_r($banner);
    ...
    function createproductsubmit()
	{
		...
		else
		{
			...
			if($this->product_model->create($name,$alias,$shop,$stock,$ean,$tax,$metatitle,$metadescription,$shopnavigation,$tags,$attribute,$isdefault)==0)
    ...
    function viewcompany()
	{
		$access = array("1");
		$this->checkaccess($access);
		$data['table']=$this->company_model->viewcompany();
    ...
    function createcompanysubmit()
	{
		...
		else
		{
			...
			if($this->company_model->create($name,$shops,$user)==0)
    ...
    function editcompany()
	{
        ...
        $data['shops']=$this->shop_model->getshopdropdown();
        $data['user']=$this->user_model->getuserdropdown();
        $data['selectedshops']=$this->company_model->getshopsbycompany($this->input->get('id'));
...
//		$data['page2']='block/companyblock';
    ...
    //		$data['page2']='block/companyblock';
		...
		else
		{
			...
			if($this->company_model->edit($id,$name,$shops,$user)==0)
			$data['alerterror']="company Editing was unsuccesful";
			else
			$data['alertsuccess']="company edited Successfully.";
	...
	function deletecompany()
	{
		$access = array("1");
		$this->checkaccess($access);
		$this->company_model->deletecompany($this->input->get('id'));
		$data['table']=$this->company_model->viewcompany();
		$data['alertsuccess']="company Deleted Successfully";
		$data['page']='viewcompany';
		$data['title']='View company';
		$this->load->view('template',$data);
	}
